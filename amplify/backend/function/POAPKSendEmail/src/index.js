...
exports.handler = async (event) => {
  ...
  const projectDetails = event.Records[0].dynamodb.NewImage.projectDetails.S
  const email = event.Records[0].dynamodb.NewImage.email.S
  const companyName = event.Records[0].dynamodb.NewImage.companyName.S
  const budget = event.Records[0].dynamodb.NewImage.budget.S
  // const engagement = event.Records[0].dynamodb.NewImage.engagement.S
  // const teamStrength = event.Records[0].dynamodb.NewImage.teamStrength.S
  ...
  let userEmailBody = {
    ...
    Thank you very much for your inquiry to IDENBRID.
